...
# if [[ -e  "$ROOT_BRAVE_PATH/Default/History_bak" ]]
# then
#   cp "$ROOT_BRAVE_PATH/Default/History" "$ROOT_BRAVE_PATH/Default/History_bak"
# fi

# HISTORY_DB="$ROOT_BRAVE_PATH/Default/History_bak"

# # # now we add the history selection
# SQL="SELECT u.title, u.url FROM urls as u WHERE u.url LIKE 'https%';"
# histlist=$(printf '%s\n' "$(sqlite3 "$HISTORY_DB" "$SQL")" | awk -F "|" '{print $1" # "$NF} ' )

# choice=$(printf '%s\n' "$SEPARATOR" "${!BOOKMARKS[@]}" \
#    "$SEPARATOR" "$histlist" | rofi -matching normal -dmenu -i -l 9 -p \
#    "open in brave: ")

choice=$(printf '%s\n' "$SEPARATOR" "${!BOOKMARKS[@]}" \
  | rofi -matching normal -dmenu -i -l 9 -p \
    "$BRAVE_DISPLAY_TEXT")
...
#   brave "$BRAVE_ARGS" "$url"
  ...
  case "$SEARCH_ENG" in
    !g)
      brave "$BRAVE_ARGS" "https://google.com/search?hl=en&q=$SEARCH_QUERY"
      ;;
    !gt)
